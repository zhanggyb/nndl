...
梯度的快速算法，也就是\emph{反向传播}\index{反向传播}。
...
例如，下图给出了第二隐藏层的第四个神经元到第三隐藏层的第二个神经元的链接上的权
重：
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(m0) [circle,draw,right=1.5 of l0] {};
    \node(m1) [circle,draw,above=0.6 of m0] {};
    \node(m2) [circle,draw,above=0.6 of m1] {};
    \node(m3) [circle,draw,above=0.6 of m2] {};
    
    \node(l2) [circle,draw,left=1.5 of m3] {};
    
    \node(r0) [circle,draw,right=1.5 of m1] {};
    \node(r1) [circle,draw,right=1.5 of m2] {};
    
    \coordinate (lc) at ($(l0)!0.5!(l2)$);
    \node(l1) at (lc) [circle,draw] {};
    
    \node(tl) [above=0.1 of l2] {\footnotesize layer 1};
    \node(tm) [above=0.1 of m3] {\footnotesize layer 2};
    ...
    \node(tr) at (\posx,\posy) {\footnotesize layer 3};
    
    \coordinate (rc) at ($(r0)!0.5!(r1)$);
    
    \node(desc) [right=0.5 of rc] {
      \begin{tabular}{l}
        {\footnotesize $w^l_{jk}$ 是从 $(l - 1)^{\rm th}$ 层的 $k^{\rm th}$ 个神经元}\\
        {\footnotesize 到 $l^{\rm th}$ 层的 $j^{\rm th}$ 个神经元的链接上的权重}\\
      \end{tabular}
    };
    
    \coordinate (bc) at ($(m0)!0.5!(r0)$);
    
    \node(tw) at (3.8,4.2) [blue] {\footnotesize $w_{24}^3$};
...
我们对网络的偏差和激活值也会使用类似的表示。显式地，我们使用 $b^l_j$ 表示
在 $l^{\rm th}$ 层 $j^{\rm th}$ 个神经元的偏差，使用 $a^l_j$ 表示 $l^{\rm
  th}$ 层 $j^{\rm th}$ 个神经元的激活值。下面的图清楚地解释了这样表示的含义：
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(m0) [circle,draw,right=1.5 of l0] {};
    \node(m1) [circle,draw,above=0.6 of m0] {\footnotesize $b_3^2$};
    \node(m2) [circle,draw,above=0.6 of m1] {};
    \node(m3) [circle,draw,above=0.6 of m2] {};
    ...
    \node(r0) [circle,draw,right=1.5 of m1] {};
    \node(r1) [circle,draw,right=1.5 of m2] {\footnotesize $a_1^3$};
...
有了这些表示， $l^{\rm th}$ 层的 $j^{\rm th}$ 个神经元的激活
值 $a^{l}_j$ 就和 $(l-1)^{\rm th}$ 层关联起来了（对比公式~\eqref{eq:4} 和上一章的
讨论）。
...
其中求和是在 $(l-1)^{\rm th}$ 层的所有 $k$ 个神经元上进行的。为了用矩阵的形式重写
这个表达式，我们对每一层 $l$ 都定义一个\emph{权重矩阵} $w^l$ \index{权重矩阵}。权
...
\begin{center}
  \begin{tikzpicture}
    ...
    \coordinate (rc) at ($(r0)!0.5!(r1)$);
    \node(eq) [right=1.5 of rc] {\footnotesize cost $C = C(a^L)$};
...
例如，二次代价函数满足这个要求，因为对于一个单独的训练样本 $x$ 其二次代价函数可以
写作：
...
出 $y$ 同样是一个固定的参数。尤其是它并不是可以随意通过改变权重和偏差来改变的，也
就是说，这不是神经网络学习的对象。所以，将 $C$ 看成仅有输出激活值 $a^L$ 的函数才
是合理的，而 $y$ 仅仅是帮助定义函数的参数而已。
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(r0) [circle,draw,right=1.5 of m0] {};
    \node(r2) [circle,draw,right=1.5 of m3] {};
    
    \coordinate (lc) at ($(l0)!0.5!(l2)$);
    \node(l1) at (lc) [circle,draw] {};
    
    \coordinate (rc) at ($(r0)!0.5!(r2)$);
    \node(r1) at (rc) [circle,draw] {};
    
    \node(t) at (3.8,5) [blue] {\footnotesize neutron $j$, layer $l$};
    \draw[->,thick,blue] (t) to (m1);
    
    \foreach \x in {0,1,2}
    \node(tl\x) [left=0.1 of l\x] {\footnotesize $\ldots$};
    
    \foreach \x in {0,1,2}
    \node(tr\x) [right=0.1 of r\x] {\footnotesize $\ldots$};
    
    \node (n0) [circle,draw,right=5 of m1] {};
    \node (n1) [circle,draw,right=5 of m2] {};
    \coordinate (nc) at ($(n0)!0.5!(n1)$);
    \node (c) [right=1.5 of nc] {$C$};
    \node(tn0) [left=0.1 of n0] {\footnotesize $\ldots$};
    \node(tn1) [left=0.1 of n1] {\footnotesize $\ldots$};
...
这个调皮鬼在 $l$ 层的第 $j^{th}$ 个神经元上。当输入进来时，调皮鬼对神经元的操作进
行搅局。他会增加很小的变化 $\Delta z_j^l$ 在神经元的带权输入上，使得神经元输出
由 $\sigma(z_j^l)$ 变成$\sigma(z_j^l + \Delta z_j^l)$。这个变化会向网络后面的层进
行传播，最终导致整个代价函数产生 $\frac{\partial C}{\partial z_j^l} \Delta
z_j^l$ 的改变。

现在，这个调皮鬼变好了，试着帮助你来优化代价函数，他们试着找到可以让代价更小
...
这是一个非常自然的表达式。右式第一个项 $\partial C/\partial a_j^L$ 表示代价随
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(i) [circle,draw] {};
    \node(o) [circle,draw,right=2 of i] {};
    
    \draw[->] (i) -- node [above] {
      \footnotesize $\frac{\partial C}{\partial w} = a_{\rm in} \delta_{\rm out}$
    } (o);
...
经元已经\emph{饱和}\index{饱和}了，并且，权重学习也会终止（或者学习非常缓慢）。类
似的结果对于输出神经元的偏差也是成立的。
...
\begin{center}
  \begin{minipage}{0.7\textwidth}
    \begin{center}
      \begin{framed}
        \textbf{总结：反向传播的四个方程式}\index{反向传播的四个方程式}\\
        \vspace{1.5ex}
        \begin{tabular}{ll}
...
分的\href{https://en.wikipedia.org/wiki/Chain_rule}{链式法则}\index{链式法则}的推
...
\begin{align}
  \label{eq:40}\delta^l_j = & \frac{\partial C}{\partial z^l_j} \tag{40}\\
  \label{eq:41}           = & \sum_k \frac{\partial C}{\partial z^{l+1}_k} \frac{\partial z^{l+1}_k}{\partial z^l_j} \tag{41}\\ 
  \label{eq:42}           = & \sum_k \frac{\partial z^{l+1}_k}{\partial z^l_j} \delta^{l+1}_k\tag{42}
\end{align}
...
检视这个算法，你可以看到为何它被称作\emph{反向传播}。我们从最后一层开始向后计算误
...
反向传播聪明的地方就是它确保我们可以同时计算\emph{所有的}偏导数
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(t) [blue,above=1.5 of p] {\footnotesize $\Delta w^l_{jk}$};
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(t) [blue,above=0.1 of m3] {\footnotesize $\Delta a^l_j$};
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(r0) [circle,draw,right=1.5 of m0] {\footnotesize $a^{l+1}_q$};
    \node(r2) [circle,draw,right=1.5 of m3] {};
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(q0) [above=1.5 of p0,blue] {\footnotesize $\frac{\partial a^l_j}{\partial w^l_{jk}}$};
    \node(q1) [above=1.5 of p3,blue] {\footnotesize $\frac{\partial a^{(l+1)}_q}{\partial a^l_j}$};
    \node(q2) [above=of p2,blue] {\footnotesize $\frac{\partial C}{\partial a^L_m}$};
...
化。进过几次迭代证明改进后，你会发现最终的简单却看起来奇特的证明\footnote{需要一
  ...
  妙之处是改用加权的输入，例如
  $z^{l+1}_q$，作为中间变量。如果你想不到这个主意，而是继续使用激活值$a_q^{l+1}$，
  你得到的证明最后会会比本章前面给出的证明稍微复杂些。}，因为你移除了很多构造的细
节了！老实告诉你，其实最早的证明的出现也不是太过神秘的事情。因为那只是很多对简化
证明的艰辛工作的积累。
