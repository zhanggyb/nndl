...
\begin{center}
  \begin{tikzpicture}
    ...
    \node(desc) [right=0.5 of rc] {
      \begin{tabular}{l}
        {\footnotesize $w^l_{jk}$ 是从 $(l - 1)^{\rm th}$ 层的第 $k^{\rm th}$ 个神经元}\\
        {\footnotesize 到 $l^{\rm th}$ 层的第 $j^{\rm th}$ 个神经元的链接上的权重}\\
      \end{tabular}
    };
...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw[->] (r0) to node [above,yshift=-1mm] {\footnotesize $a^L_2$} (eq);
    \draw[->] (r1) to node [above,yshift=-1mm] {\footnotesize $a^L_1$} (eq);
...
\hyperref[fig:sigmoid_graph]{上一章中S型函数的图形}，当 $\sigma(z^L_j)$ 近似为
