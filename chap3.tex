...
最终在验证集上的准确度达到了 97.92\%。这是比 30 个隐藏元的较大飞跃。
\label{chap3_98_04_percent}实际上，稍微改变一点，60 \epochs{} $\eta=0.1$ 和
$\lambda = 5.0$。我们就突破了 98\%，达到了98.04\% 的分类准确度\label{98percent}。
对于 152 行代码这个效果还真不错！

我已经把规范化描述为一种减轻过度拟合和提高分类准确度的方法。实际上，这不是仅有的
...
布来对偏差进行初始化，后面会告诉你原因。有了这些设定，带权和 $z = \sum_j w_j x_j
...
均值为 $0$ 而标准差为 $1$ 的高斯分布来初始化偏差。下面是代码：
\begin{lstlisting}[language=Python]
