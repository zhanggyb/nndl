...
方便对比\footnote{这个图形由程序 \lstinline!more_data.py! 生成（和上面几个图形一
    样）。}：
...
有\footnote{显著的例子在\href{http://dx.doi.org/10.3115/1073012.1073017}{Scaling
...
行初始化。提醒你一下，之前的方式就是根据独立的均值为 $0$，标准差为$1$ 的高斯随机
变量随机采样作为权重和偏差的初始值。这个方法工作的还不错，但是非常\emph{特别}，
所以我们需要寻找一些更好的方式来设置我们网络的初始化权重和偏差，这对于帮助网络学
习速度的提升很有价值。

结果表明，我们可以比使用归一化的高斯分布效果更好。为什么？假设我们使用一个很多的
