...
不是有用的学习。我们说网络在 280 \epoch{}后就\emph{过匹配}\index{过匹
  配}（overfitting\index{overfitting}）或者\emph{过度训练}\index{过度训练}（overtraining\index{overtraining}）
了。
...
据集上的代价表现是越来越好的。这其实是另一种模型过匹配的迹象。尽管，这里带来了关
于我们应当将 15 还是 280 \epoch{}当作是过匹配开始影响学习的时间点的困扰。从一个
实践角度，我们真的关心的是提升测试数据集上的分类准确度，而测试集合上的代价不过是
分类准确度的一个反应。所以更加合理的选择就是将 280 \epoch{}看成是过匹配开始影响学习的
时间点。
...
\lstinline!test_data! 来防止过匹配。我们会为 \lstinline!test_data! 使用和上面提
...
程。如果我们这样做，难道不会产生过匹配于 \lstinline!test_data! 的困境么？我们是
...
我们已经研究了只使用 $1,000$ 幅训练图像时的过匹配问题。那么如果我们使用所有的
...
1.53\%。而之前的例子中，这个差距是 17.73\%！过匹配仍然发生了，但是已经减轻了不少。
我们的网络从训练数据上更好地泛化到了测试数据上。一般来说，最好的降低过匹配的方式
之一就是增加训练样本的量。有了足够的训练数据，就算是一个规模非常大的网络也不大容
易过匹配。不幸的是，训练数据其实是很难或者很昂贵的资源，所以这不是一种太切实际的
选择。
...
显然，规范化的使用能够解决过匹配的问题。而且，准确度相当高了，最高处达到了
...
会发生什么？当然，我们之前已经看到过匹配在大规模的数据上其实不是那么明显了。那规
...
我已经把规范化描述为一种减轻过匹配和提高分类准确度的方法。实际上，这不是仅
...
合上训练 30 个\epochs{}，然后会随着训练数据量的下降而成比例变化\epochs{}数。为了确保权重下
降因子在训练数据集上相同，我们会在全部训练集上使用规范化参数为 $\lambda = 5.0$，
然后在使用更小的训练集的时候成比例地下降 $\lambda$ 值\footnote{这个以及下两个图形由程序
...
中介绍的那样进行初始化。提醒你一下，之前的方式就是根据独立的均值为 $0$，标准差为
...
**宽的策略**：在使用神经网络来解决新的问题时，一个挑战就是获得**任何**一种非寻常
...
**\learningrate{}**：假设我们运行了三个不同\learningrate{}（$\eta=0.025$、$\eta=0.25$、$\eta=2.5$）
...
**使用 Early stopping 来确定训练的回合数**：正如我们在本章前面讨论的那样，Early
...
够帮助我们避免过匹配。尽管在实验前期不采用 Early stopping，这样可以看到任何过匹
...
**\learningrate{}调整**：我们一直都将\learningrate{}设置为常量。但是，通常采用可变的\learningrate{}更加有效。
...
**规范化参数**：我建议，开始时不包含规范化（$\lambda=0.0$），确定 $\eta$ 的值。
...
**在本书前面，我是如何选择超参数的**：如果你使用本节给出的推荐策略，你会发现你自
...
**minibatch 大小**：我们应该如何设置 minibatch 的大小？为了回答这个问题，让我们
...
**总结**：跟随上面的经验并不能帮助你的网络给出绝对最优的结果。但是很可能给你一个
...
\begin{equation}
  ...
  \label{eq:104}\tag{104}
\end{equation}
其中 $\nabla C$ 是通常的梯度向量，$H$ 就是矩阵形式的 Hessian 矩阵，其中 $jk$-th
...
另一个变体就是 \emph{Rectified Linear 神经元}或者 \emph{Rectified Linear Unit}，
...
\begin{quote}
{\bfseries\itshape 问题：}你怎么看那些全部由实验效果支撑（而非数学保证）的使用和
...
-- {\itshape
  \href{http://www.reddit.com/r/MachineLearning/comments/25lnbt/ama_yann_lecun/chivdv7}{
    问题与回答} with neural networks researcher Yann LeCun}
\end{quote}
