...
\begin{document}
  \begin{tikzpicture}[
    ...
    \begin{scope}
      ...
      \node [blue,above] at (n.north) {$b = -0.5$};

      \draw[->] (h0) -- node [yshift=-2mm] {\scriptsize $-0.3$} (n);
      \draw[->] (h1) -- node [yshift=2mm] {\scriptsize $0.3$} (n);
      \draw[->] (h2) -- node [yshift=2mm] {\scriptsize $0.3$} (n);
      \draw[->] (h3) -- node [yshift=-2mm] {\scriptsize $-0.3$} (n);
    ...
    \begin{scope}[xshift=2cm,yshift=-2.5cm]    
      ...
      \begin{axis}[
        ...
        declare function={
          sigma(\z)=1/(1 + exp(-\z));
 		  f(\x,\y)=sigma(-0.5 + 0.3 * (sigma(1000 * (\x - 0.4)) - sigma(1000 * (\x - 0.6)))	+ 0.3 * (sigma(1000 * (\y - 0.3)) - sigma(1000 * (\y - 0.7))));
