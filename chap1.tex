...
得方程~\eqref{eq:6} 的代价取得最小值的权重 $w_k$ 和偏差 $b_l$。为了清楚这是如何
工作的，我们将用权重和偏差代替变量 $v_j$。也就是说，现在``位置''变量有两个分量组
...
\emph{\minibatch{}\index{\minibatch}（mini-batch\index{mini-batch}）}。假设样本
...
了一个训练\emph{迭代期\index{迭代期}（epoch\index{epoch}）}。然后我们就会开始一
个新的训练\epoch{}。

另外值得提一下，对于改变代价函数大小的参数，和用于计算权重和偏差的\minibatch{}的
...
\item 梯度下降算法一个极端的版本是把\minibatch{}的大小设为 $1$。即，假设一个训练
  ...
  复。这个过程被称为 \emph{online}、\emph{on-line}、或者\emph{递增}学习。在
...
被称为超参数，以区别于通过我们的学习算法所学到的参数（权重和偏差）。如果我们选择
...
中的其它每一个部分。我们可能想知道是否用了让网络很难学习的初始权重和偏差？或者可
